  ...
  initFormList = () => {
    ...
    const uploadProps = {
      ...
      onChange(info) {
        ...
        const filename =
          info.fileList[0].response && info.fileList[0].response.data && info.fileList[0].response.data.filename
        ...
        if (info.file.status === 'done') {
          if (info.file.response && info.file.response.code === 200) {
            ...
            message.error(
              `${info.file.name} 解析失败：${
                (info.file.response && info.file.response.msg) || (info.file && info.file.response.errorMsg)
              }`
            )
          }
          ...
          const UPLOAD = (
            <Row>
              ...
              >
                {detail ? (
                  <img
                    src={initialValue && initialValue[0] && initialValue[0].url}
                    alt=""
