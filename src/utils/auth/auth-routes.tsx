...
export type AuthNonIndexRouteObject<T extends Record<string, any> = any> = Omit<NonIndexRouteObject, 'children'> & {
  [P in keyof T]: T[P]
} & {
  auth?: string | string[]
...
export type AuthRouteObject<T extends Record<string, any> = any> = AuthIndexRouteObject<T> | AuthNonIndexRouteObject<T>
      ...
      if (!hasAuth) {
        ...
        _router = {
          ...
          element: React.cloneElement(router.element, {
            ...
            ...(router.genAuthRoutersProp ? { authRouters: getMenus(router.children) } : {}),
