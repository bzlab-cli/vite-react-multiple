          ...
          const beginTime = (
            <FormItem label="选择日期" key="beginTime">
              {getFieldDecorator('beginTime')(
                <DatePicker showTime style={width} placeholder="开始时间" format="YYYY-MM-DD" />
              )}
            </FormItem>
          )
          ...
          const endTime = (
            <FormItem key="endTime">
              {getFieldDecorator('endTime')(
                <DatePicker showTime style={width} placeholder="结束时间" format="YYYY-MM-DD" />
              )}
            </FormItem>
          )
  ...
  render() {
    return (
      <Form layout={'inline'}>
        {this.initFormList()}
        <FormItem>
          <Button type="primary" onClick={this.onSubmit} style={{ margin: '0 20px' }}>
            提交
          </Button>
          ...
          {this.props.nameList.map(item => {
            ...
            if (item === 'add') {
              domlist.push(
                <Button key={item} type="primary" style={{ margin: '0 0 0 50px' }} onClick={this.props.handleAdd}>
                  新增
                </Button>
              )
            } else if (item === 'patch') {
              domlist.push(
                <Popconfirm title="确定删除？" onConfirm={this.pitchDelete}>
                  <Button key={item} disabled={!(selectedRowKeys && selectedRowKeys.length > 0)} type="primary">
                    删除
                  </Button>
                </Popconfirm>
              )
